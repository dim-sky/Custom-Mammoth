...
class FrozenBackboneKACLwF(FrozenBackboneKACPretrained):
    ...
    def __init__(self, backbone, loss, args, transform, dataset):
        # Call KAC parent to get KAC classifier
        ...
        print(f"\n{'='*70}")
        print(f"[LwF-KAC] Learning without Forgetting enabled")
        print(f"[LwF-KAC] Classifier type: {type(self.net.classifier)}")
        print(f"{'='*70}\n")
