"""
Frozen Backbone + KAC Classifier + EWC
"""
...
@register_model('frozen_backbone_kac_ewc_pretrained')
class FrozenBackboneKACEWCPretrained(EWCMixin, FrozenBackboneKACPretrained):
    """Frozen Backbone + KAC + EWC"""
    NAME = 'frozen_backbone_kac_ewc_pretrained'
    COMPATIBILITY = ['class-il', 'task-il']
    
    def __init__(self, backbone, loss, args, transform, dataset):
        EWCMixin.__init__(self)
        FrozenBackboneKACPretrained.__init__(self, backbone, loss, args, transform, dataset)
        
        self.ewc_lambda = getattr(self.args, 'ewc_lambda', 1000.0)
        print(f"[KACEWC] EWC lambda: {self.ewc_lambda}")
