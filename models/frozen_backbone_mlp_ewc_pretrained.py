"""
Frozen Backbone + MLP Classifier + EWC
"""
...
@register_model('frozen_backbone_mlp_ewc_pretrained')
class FrozenBackboneMLPEWCPretrained(EWCMixin, FrozenBackboneMLPPretrained):
    """Frozen Backbone + MLP + EWC"""
    NAME = 'frozen_backbone_mlp_ewc_pretrained'
    COMPATIBILITY = ['class-il', 'task-il']
    
    def __init__(self, backbone, loss, args, transform, dataset):
        EWCMixin.__init__(self)
        FrozenBackboneMLPPretrained.__init__(self, backbone, loss, args, transform, dataset)
        
        self.ewc_lambda = getattr(self.args, 'ewc_lambda', 1000.0)
        print(f"[MLPEWC] EWC lambda: {self.ewc_lambda}")
    ...
    def get_parser(parser):
        parser = FrozenBackboneMLPPretrained.get_parser(parser)
